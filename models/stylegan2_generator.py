...
from .stylegan2_generator_model import StyleGAN2GeneratorModel
...
class StyleGAN2Generator(BaseGenerator):
  ...
  def synthesize(self,
    ...
    if latent_space_type == 'Z':
      ...
      wps = self.model.truncation(ws, trunc_psi=0.5, trunc_layers = 18)
      results['z'] = latent_codes
      results['w'] = self.get_value(ws['w'])
    ...
    if generate_image:
      ...
      results['image'] = self.get_value(images['image'])
